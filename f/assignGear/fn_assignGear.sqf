...
diag_log format ["Init loadouts... Faction %1 Server status is %2",_faction,isServer];
if (!(_unit isKindOF "CAManBase")) then {_faction = "rhs_faction_msv";};
if (isServer && !(_unit getvariable ["bg_gearset", false])) then {
	diag_log format ["Init loadouts... Running Faction %1",_faction];
...
{ // Check for JIP from standard slots.
	diag_log format ["Init loadouts... JIP Faction %1",_faction];
	if (local _unit) then
	{
